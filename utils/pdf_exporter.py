    ...
    try:
        ...
        if isinstance(text, bytes):
            try:
                ...
                print(f"Warning: Invalid UTF-8 detected, using latin1 fallback: {text[:30]}")
                return text.decode("latin1", errors="replace")
        if not isinstance(text, str):
            text = str(text)
        return text.encode("utf-8", errors="replace").decode("utf-8", errors="replace")
    except Exception as e:
        print(f"Encoding Error: {e} for text: {repr(text)}")
        return f"[ENCODING_ERROR: {str(e)}]"
    ...
    for para in paragraphs:
        ...
        if para:
            ...
            for line in wrapped_lines:
                pdf.cell(0, 10, clean_encoding(line), ln=True)
            pdf.ln(2)
...
def generate_pdf_report(session, filename="summary.pdf", model_source="groq"):
    df = session["df"]
    dataset_name = clean_encoding(session.get("name", "Unnamed Dataset"))
    ...
    if insights:
        for idx, insight in enumerate(insights, start=1):
            ...
            pdf.multi_cell(0, 10, clean_encoding(f"5.{idx} Insight: {insight['question']}"))
            pdf.set_font("DejaVu", "", 12)
    ...
    if recommendations:
     ...
     for idx, rec in enumerate(recommendations, start=1):
        text = (
            f"Insight: {clean_encoding(rec.get('Insight', 'N/A'))}\n"
            f"Recommended Action: {clean_encoding(rec.get('Recommended Action', 'N/A'))}\n"
            f"Priority: {clean_encoding(rec.get('Priority', 'N/A'))}\n"
            f"Owner/Team: {clean_encoding(rec.get('Owner', 'N/A'))}\n"
            f"Timeline: {clean_encoding(rec.get('Timeline', 'N/A'))}\n"
            "----------------------------------------------------\n"
        )
...
def export_to_pptx(session, filename="summary.pptx"):
    ...
    chat_text = ""
    for chat in session.get('chat_history', []):
        user = clean_encoding(chat.get('user', ''))
        assistant = clean_encoding(chat.get('assistant', {}).get('response', ''))
        chat_text += f"{user}\n{assistant}\n\n"
